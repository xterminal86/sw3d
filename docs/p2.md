...
Which is almost what we need. And since we conveniently settled on `CW` order, all that is left for us to do is to perform only one swap of vertices `2` and `3` for `FB` triangle: 
...
Don't know about you, but when I started researching topic of software 3D rendering, this was the first time I learned about existence of such rule. Like, I never actually gave it any thought, because if you draw stuff with one color (and that's probably what you're going to do in the beginning) everything looks fine. So you might think that, you know, everything works, and move on to next topics. And then imagine you get to blending and then suddenly everything breaks for some reason. Talking about small details...
...
Actually in our scanline rasterizer it's super easy: 

1. Draw left and top edge for FT triangle and don't draw right.

2. Draw left edge for FB triangle and don't draw bottom.
...
There's one thing, though: the result is obviously not very good. XD
...
And we can't do anything about it actually. Because all these triangles start from one point, somebody has to draw _somewhere_, while others have to skip it. If we draw all of them it _probably_ works, I guess:

![](images/p2/tlr-ok-i-guess.png)

So, in isolation one triangle might not actually be rasterized correctly.

But actually, I just realized, that all of this probably wasn't necessary, since we're going to implement `Z buffer`, which will handle this overdrawing problem for us by **not** drawing a pixel if there's already one present in the Z buffer (with depth value that doesn't satisfy depth test condition of course).

Oh well... `¯\_(ツ)_/¯`

Anyway, you can check out the [project](https://github.com/xterminal86/sw3d/tree/main/tests/top-left-rule), if you like.
